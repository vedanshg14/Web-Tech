...
const EatsOrders = () => {
  const [orders, setOrders] = useState<EatsOrder[]>([]);
  ...
  useEffect(() => {
    const fetchOrders = async () => {
      try {
        const ordersRef = collection(db, "eatsOrders");
        ...
        console.error("Error fetching eats orders:", error);
      } finally {
        setLoading(false);
      }
    };
  ...
  const updateOrderStatus = async (orderId: string, newStatus: string) => {
    try {
      const orderRef = doc(db, "eatsOrders", orderId);
